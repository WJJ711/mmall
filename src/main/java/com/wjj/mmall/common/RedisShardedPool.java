...
public class RedisShardedPool {
    ...
    private static Boolean testOnBorrow = Boolean.parseBoolean(PropertiesUtil.getProperty("testOnBorrow","true"));
    //在return一个jedis实例的时候，是否要进行验证操作，如果赋值为true，则放回jedispool的jedis实例肯定是可以用的。
    private static Boolean testOnReturn= Boolean.parseBoolean(PropertiesUtil.getProperty("testOnReturn","true"));
    ...
    private static void initPool(){
        ...
        //连接耗尽，是否阻塞，false会抛出异常，true阻塞直到超时，默认为true
        config.setBlockWhenExhausted(true);

        //设置超时时间为2s
        JedisShardInfo info1 = new JedisShardInfo(redis_1_Ip, redis_1_Port, 1000 * 2);
        JedisShardInfo info2 = new JedisShardInfo(redis_2_Ip, redis_2_Port, 1000 * 2);
